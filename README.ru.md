...
`/etc/init.d/dns-box`, бинарь запускается из `/tmp/dns-box/dns-box`, конфиг —
из `/data/dns-box/config.json` (`/tmp` на роутере очищается при ребуте, `/data`
переживает). Постоянная копия бинаря лежит в `/data/dns-box/dns-box`: на каждом
старте `try_local` возвращает её в `/tmp`, если там пусто, — роутер, поднявшийся
без интернета, всё равно получает рабочий резолвер. Копия обновляется после
каждого успешного апдейта и никогда не затирает бинарь, который уже лежит
в `/tmp`, иначе каждый рестарт откатывал бы свежескачанную версию.
...
свежий бинарь. Закачка идёт во временный файл, и подмена происходит только
после того, как `-version` докажет, что скачанное вообще запускается; неудачное
обновление не фатально — сервис стартует на том бинаре, который уже есть.
Ассет релиза выбирается по `uname -m` (`asset_name`), так что aarch64-роутер
получает `dns-box-linux-arm64`, а не 32-битную softfloat-сборку.

При `AUTO_UPDATE=0` (дефолт) не качается ничего, и сервис работает с тем, что
лежит в `/data/dns-box/dns-box`, — доставлять бинарь нужно самому. Учти, что
`make copyToRouter` кладёт его в `/tmp/dns-box/`: скопируй и в
`/data/dns-box/dns-box`, иначе после ребута его не станет.
...
#### `local` — замена DNS-части dnsmasq

Чтобы убрать двойной хоп, dnsmasq переводится в режим «только DHCP», а
dns-box садится на 53-й порт. dns-box тогда сам отвечает за локальные имена:

| Параметр | Тип | Описание |
|-----------|-----|----------|
| `enabled` | `bool` | Отвечать локальными именами и PTR |
| `leases_files` | `[]string` | Lease-файлы dnsmasq, дефолт `["/tmp/dhcp.leases"]`. Формат: `<expires> <mac> <ip> <name> <clientid>`; записи с именем `*` пропускаются |
| `hosts_files` | `[]string` | Файлы в hosts-формате (odhcpd), дефолт `["/tmp/hosts/odhcpd"]` |
| `domain` | `string` | Поисковый домен (например `lan`): клиенты доступны и как `nas`, и как `nas.lan` |

Файлы перечитываются по mtime раз в 10 секунд; исчезнувший файл опустошает
зону, а не консервирует старые записи. A/AAAA-ответы отдаются с
фиксированным TTL 60 секунд. Зона авторитетна в обе стороны (аналог
`local=/lan/` в dnsmasq): незнакомые имена под поисковым доменом и
не-A/AAAA типы локальных имён получают локальный NXDOMAIN и не утекают
к апстрим-резолверам. PTR локальных диапазонов (RFC 1918/4193, RFC 6598
CGNAT — включая Tailscale 100.64/10, loopback, link-local) также всегда
обслуживается локально — из lease-данных или NXDOMAIN.

**ACL.** При переезде на `:53` обязательно ограничьте, кому отвечаем:
...
Клиенты вне этих подсетей получают REFUSED. Пустой список разрешает всем;
wildcard-бинд (`0.0.0.0`/`::`) с пустым ACL логирует предупреждение при
старте. Привязывайтесь к конкретным адресам интерфейсов, а не к wildcard.

**Чек-лист миграции (OpenWrt):**

1. Настройте `local`, `allowed_subnets` и явные адреса бинда, как выше.
2. `uci set dhcp.@dnsmasq[0].port='0'; uci commit dhcp` — dnsmasq ведёт DHCP,
   DNS больше не отвечает.
3. Перезапустите dns-box на 53-м порту; проверьте:
   `dig @192.168.1.1 nas.lan`, `dig @192.168.1.1 -x 192.168.1.10`,
   и снаружи LAN сервер должен REFUSE.
